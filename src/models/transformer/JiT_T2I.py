...
class JiT_T2I(nn.Module):
    ...
    def _build_text_condition(self, text_t, batch_size):
        text_condition = self.t_embedder(text_t)
        if text_condition.ndim == 2:
            text_condition = text_condition.view(batch_size, 1, self.hidden_size)
        elif text_condition.ndim != 3 or text_condition.shape[1] != 1:
            raise ValueError(
                f"text_t must embed to [B, D] or [B, 1, D], got {text_condition.shape}"
            )
        return nn.functional.silu(text_condition)

    def _build_img_condition(self, t, batch_size, num_img_tokens):
        img_condition = self.t_embedder(t)
        if img_condition.ndim == 2:
            img_condition = img_condition.view(batch_size, 1, self.hidden_size)
        elif img_condition.ndim == 3:
            if img_condition.shape[1] not in (1, num_img_tokens):
                raise ValueError(
                    f"Token-wise timestep shape {img_condition.shape} does not match image token count {num_img_tokens}"
                )
        else:
            raise ValueError(
                f"t must embed to [B, D] or [B, N, D], got {img_condition.shape}"
            )
        return nn.functional.silu(img_condition)

    def forward(self, x, t, y, text_t=None, return_layer=None, return_after_layer=None):
        if return_layer is not None and return_after_layer is not None:
            raise ValueError("Use either return_layer or return_after_layer, not both")
        ...
        text_condition = self._build_text_condition(text_t, B)
        img_condition = self._build_img_condition(t, B, num_img_tokens)
        y = self.y_embedder(y).view(B, -1, self.hidden_size) + ypos.to(y.dtype)
        for i, block in enumerate(self.text_refine_blocks):
            y = block(y, text_condition)
        ...
        for i, block in enumerate(self.blocks):
            if return_layer is not None and i==return_layer:
                feat = x
            x = block(x, y, img_condition, xpos)
            if return_after_layer is not None and (i + 1) == return_after_layer:
                feat = x
        ...
        if return_layer is not None or return_after_layer is not None:
            if feat is None:
                raise ValueError("Requested feature layer was not found")
            return output, feat
        else:
            return output